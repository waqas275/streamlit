...
st.set_page_config(page_title="Sales Dashboard Example", page_icon=":bar_chart:", layout="wide")
...
testdf = pd.pivot_table(df_selection, values =["Revenue"],index =["State","Product"],aggfunc='count').reset_index()
heatmap = pd.pivot(testdf, index="State", columns="Product", values="Revenue")
heatmapchart = px.imshow(heatmap,height=1400,width=1200,title="Product & States Heatmap",color_continuous_scale = 'Reds',labels=dict(color="Orders"))	
